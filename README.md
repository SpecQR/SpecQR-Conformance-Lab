...
- Canvas / browser helper の自動検証はこの Node-only suite では実装しない。
- browser helper は Node 上で import / type check し、browser automation はこの suite では実行しない。

contributor 向けの実装制約は [docs/development-policy.md](docs/development-policy.md) にまとめています。

## Governance / maintenance

contribution、security、dependency、release readiness の運用方針は次の文書にまとめています。

- [CONTRIBUTING.md](CONTRIBUTING.md): vector / adapter 追加、required checks、expected skip / failure semantics、maintainer identity。
- [SECURITY.md](SECURITY.md): security issue の報告方法、workflow / token / publishing path 変更の扱い、report/data hygiene。
...
`operation` は QR generation、manual segments、Planning API、GS1 / Digital Link helper、Structured Append helper、Package Surface check を表せる enum です。adapter 実装者はこの schema を先に読めば、どの入力をどの API に渡し、どの期待値を比較すべきか分かる状態を目指します。
...
- SpecQR: 公開 npm package `specqr@2.4.0` を対象にする。現在は `generate`, `generateSegments`, Planning / Diagnostics API、SpecQR がサポートする GS1 / Digital Link helper subset、Structured Append helper、Rendering / Output surface、Package Surface の確認に対応している。
...
Rendering / Output suite は、SpecQR adapter が published `specqr@2.4.0` の `options.output` を保って実行し、matrix shape、SVG root / dimensions / colors、PNG signature / dimensions、SVG Data URL / PNG Data URL の media type と中身を確認します。PNG readability が意味を持つ case では jsQR lane でも decode を確認します。Canvas / browser helper はこの Node-only suite の対象外です。

Package Surface suite は、published `specqr@2.4.0` の package metadata、root export、`specqr/browser` helper、`specqr/node` helper、`fixtures/typescript-consumer/tsconfig.json` の TypeScript compile を確認します。browser helper は Node 上で safe import / function availability / type check だけを行い、browser automation や DOM integration の実行は行いません。
...
`npm run summary` は生成済み `reports/latest.json` から GitHub Step Summary 用の Markdown を出力します。通常の stdout にも同じ内容を出すため、local でも CI log でも target package、vector / result 件数、adapter 別結果、GS1 / DL、Structured Append、Planning / Diagnostics、Kanji / ECI / binary、Rendering / Output、Package Surface の主要 scope、optional decoder availability を確認できます。
...
`.github/workflows/specqr-target.yml` は Actions UI から手動で SpecQR npm package spec を差し替えて実行する調査 workflow です。`package_spec` には `specqr@2.4.0`、`specqr@latest`、`specqr@next` などを指定でき、`node_version` の default は `22` です。この workflow は pinned `specqr@2.4.0` の baseline report を `reports/baseline.json` に生成したあと、`npm install --no-save --package-lock=false "$PACKAGE_SPEC"` で requested package だけを一時的に入れ替え、`package.json` と `package-lock.json` が変わっていないことを確認してから candidate report を `reports/candidate.json` に生成します。

`npm run compare:reports -- --base reports/baseline.json --candidate reports/candidate.json --json-output reports/comparison.json --markdown-output reports/comparison.md` は baseline と candidate の target metadata、summary counts、adapter summary、主要 suite summary、vector / adapter status change、failed/error に関わる check-level change を比較します。default では count difference だけで失敗せず、`--fail-on-regression` を付けた場合だけ、新しい failed/error result や required adapter の passed check 喪失を regression として nonzero exit にします。
...
npm run conformance -- --suite rendering-output --output reports/rendering-output.local.json
npm run conformance -- --suite package-surface --output reports/package-surface.local.json
